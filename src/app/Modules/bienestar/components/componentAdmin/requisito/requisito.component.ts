...
import {
  ...
  filtradoInicial,
  VISIBILITY_FILTER
} from "../../../filter/filterRequisito.model";
...
import { RequisitoUI } from "../../../BD/store/Requisito.store";
...
export class RequisitoComponent implements OnInit {
  ...
  requisitoUIActive$: Observable<RequisitoUI> | Observable<RequisitoUI[]>;
  ...
  ngOnInit() {
    ...
    this.requisitoUIActive$ = this.requisitoQuery.ui.selectActive();
    this.controlFiltrado = new FormControl(VISIBILITY_FILTER.MOSTRAR_TODO);
    this.notificacionBusService.showNotificacionSource.subscribe(
      notificacion => {
        Swal.fire({
  ...
  @memo()
  isEditLoading() {
    return this.requisitoQuery.selectUpdateLoading();
  }
  ...
  //CRUD REQUISITO
  async guardarYEditarRequisito(formsValue) {
  ...
  mostrarDatosFormularioRequisito() {
    this.requisitoSeleccionado$.subscribe(requisitoSeleccionado => {
      if (requisitoSeleccionado) {
        let tiposSeleccionado = requisitoSeleccionado.tipos.map(
          tipo => tipo.id
        );
        let serviciosSeleccionado = requisitoSeleccionado.servicios.map(
          servicio => servicio.id
        );
        this.estadoActualizarResgitrarFormularion = true;
        this.formularioRequisito.get("id").setValue(requisitoSeleccionado.id);
        this.formularioRequisito
          .get("nombre")
          .setValue(requisitoSeleccionado.nombre);
        this.formularioRequisito
          .get("descripcion")
          .setValue(requisitoSeleccionado.descripcion);
        this.formularioRequisito
          .get("tipoArchivo")
          .setValue(requisitoSeleccionado.tipoArchivo);
        this.formularioRequisito.get("tipos").setValue(tiposSeleccionado);
        this.formularioRequisito
          .get("servicios")
          .setValue(serviciosSeleccionado);
        this.formularioRequisito
          .get("prioridad")
          .patchValue(requisitoSeleccionado.prioridad ? "true" : "false");
        this.formularioRequisito
          .get("requerido")
          .patchValue(requisitoSeleccionado.requerido ? "true" : "false");
  ...
  async eliminarArchivo(idArchivo: number) {
  ...
  changeTipo(event: any) {
    let requisitoActivo: requisito = this.requisitoQuery.getEntity(
      this.requisitoQuery.getActiveId()
    );
